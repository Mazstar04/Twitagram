...
const PostImages = ({ post }) => {
    ...
    return (
        ...
        <div className="relative">
            ...
            <div className={`pics relative ${isLaptopAndAbove ? "rounded-xl overflow-hidden" : ""}`}>
                <img src={`${post.imagePaths[0]}/${Math.floor(Math.random() * 3000)}}`} alt="post" className={`post-img w-full ${isLaptopAndAbove?   "h-64  min-h-64" : "h-96  min-h-96" }`} />

                <div className={`absolute top-8 right-2 font-bold p-1 text-center text-white counter rounded-xl  ${isLaptopAndAbove ? "text-sm w-10" : "text-md w-14"}`}>
                ...
                <div className="dot justify-center flex w-full absolute bottom-2">
                    {post.imagePaths.length > 1 && post.imagePaths.map((i, index) => {
                        return <i onClick={() => { goTo(index + 1) }} key={Math.floor(Math.random() * 100)} className={`fas fa-circle mr-1 text-xss ${activeIndex == index + 1 ? "text-blue-700  relative " : "text-white "}`}></i>;
                    })}
                </div>
